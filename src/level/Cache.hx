...
class Cache extends VisualObject {
	...
	public function new(position: Vector, depth: Float) {
		super('assets/images/cache.png', position, new Vector(128, 80), 1);
		...
		Luxe.events.listen('player.interact', function(o: {object: Player}) {
			if(Math.abs(o.object.pos.x + o.object.size.x/2 - pos.x + size.x/2) > 10) return;
			texture = Luxe.resources.texture('assets/images/cacheopen.png');
		});
	}
